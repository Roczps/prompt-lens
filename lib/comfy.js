...
// via GET /view. First version is pure txt2img; reference images can be
// added later through /upload/image + an img2img graph.
...
export async function generateImageComfy(
  ...
  if (!settings.comfyCheckpoint) {
    throw new Error('未配置 ComfyUI 模型（checkpoint）。请到设置页填写模型文件名。');
  }
  const base = apiBase(settings);
  const { width, height } = comfySize(aspectRatio, imageSize);
  const workflow = buildComfyWorkflow({ prompt, width, height }, settings);
...
/** Connectivity check; also returns available checkpoints for the options UI. */
export async function testComfy(baseUrl) {
  ...
  return { checkpoints: Array.isArray(checkpoints) ? checkpoints : [] };
}
