...
async function generateViaApimart({ instruction, aspectRatio, imageSize, refs }, settings) {
  ...
  throw new Error('生成超时（5 分钟未完成），请稍后在历史里重试');
}
...
export async function generateImageOpenAI(
  ...
  if (detectProtocol(settings) === 'apimart') {
    return generateViaApimart({ instruction, aspectRatio, imageSize, refs }, settings);
  }
