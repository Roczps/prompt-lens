...
export async function planPostSet(
  ...
  const platformGuide =
    platform === 'xhs'
      ? '小红书（生活方式种草社区，真实生活感与氛围感，封面图要足够吸睛，竖版构图）'
      : 'Instagram（视觉美学社区，整体 feed 色调统一、构图讲究、高级感）';
...
export async function writePostCopy(
  ...
  const platformRules =
    platform === 'xhs'
      ? `平台：小红书。要求：
- title：吸睛标题，14-20 个字，可带 1-2 个贴合内容的 emoji，有钩子（悬念/共鸣/数字皆可），不要标题党到失真
- body：100-200 字的种草正文，口语化、有真实体验感，按内容分 3-5 个短行（用换行分隔），适当穿插 emoji，结尾带一句互动引导（提问或召唤评论）
- tags：5-8 个话题标签（中文，不带 # 号），从内容相关、场景相关、泛流量词三层搭配`
      : `平台：Instagram。要求：
- title：一句简短英文 hook（首行），10 词以内
- body：2-4 句英文 caption，轻松自然、有个人语气，可带少量 emoji，用换行分隔句组，结尾一句 call-to-action
- tags：8-12 个英文 hashtag（不带 # 号），混合大流量与小众精准标签`;
