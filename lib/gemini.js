 ...
 */
export async function planPostSet({ base64, mimeType, stylePrompt, charDesc, platform, count }, settings) {
  const platformGuide =
    platform === 'xhs'
...
{"label": "中文短标签，如 封面·全身街拍", "prompt": "英文完整生图提示词：完整的角色外貌锚定 + 统一的风格色调光线锚定 + 本张特有的场景/景别/姿势/构图描述，80-150 词"}`;
  ...
  const shots = (Array.isArray(parsed) ? parsed : parsed.shots || [])
    ...
    .map((s, i) => ({ label: String(s.label || `第${i + 1}张`), prompt: s.prompt.trim() }));
