...
自用 Chrome 插件（Manifest V3，纯原生 JS，无构建步骤）：反推网页图片的提示词，并用 Gemini / GPT-Image 生成新图、批量生成社交平台组图。灵感来自 viko.fun。当前版本 **0.7.0**，此前在 macOS 上的 Cursor 中开发，git 历史完整（12 个提交，中文提交信息）。
...
background.js     Service Worker：任务调度中枢，所有 API 调用都在这里发起
lib/
  ...
  util.js         uid / base64 / 缩略图（OffscreenCanvas, 1280px JPEG）/ friendlyGenError
content/          悬浮球内容脚本（悬停网页图片→反推）
popup/            弹窗：上传/粘贴图片入口（注意：openSidePanel 必须在用户手势同步调用）
sidepanel/        主界面：画面解构、生图控制、组图创作、角色卡、历史
options/          设置页：双渠道 API Key、模型名、默认参数
...
- gen = { id, status(running/done/error), prompt, aspectRatio, imageSize, refMode(pose/style/none), provider(gemini/openai), characterId/Name, setId/setLabel/setIndex/setTotal/setPreset（组图字段）, apimartTaskId, images:[dataUrl], error }
...
单张：startGeneration → makeGenRecord → executeGeneration（统一执行器：解析角色卡/参考图 → 按 provider 分发 → updateGen 落结果）。
...
重试：retryGeneration 重置 gen 状态后走 executeGeneration，可覆盖 provider。

## 用户使用配置（换机后需重新配置）

- API Key 存在浏览器 chrome.storage.sync，不在仓库里。PC 上加载插件后要在设置页重新填：Gemini API Key（必须，反推和策划都用它）、APIMart Key（可选，GPT-Image 渠道，baseUrl 默认 https://api.apimart.ai/v1）。
